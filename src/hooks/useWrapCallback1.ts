...
import { useKYCContract, useTradeContract } from './useContract'
 ...
 */
 ...
 export function useWrapLockVeriCallback1(
  id: string
): { wrapType: WrapType; execute?: undefined | (() => Promise<void>); inputError?: string } {
  const { chainId } = useActiveWeb3React()
  const wethContract = useKYCContract()
          ...
          sufficientBalance
            ? async () => {
              try {
                ...
                const txReceipt = await wethContract.lock(id)
                addTransaction(txReceipt, { summary: `LOCK ORDER: ${id} ` })
...
export function useWrapVerifyCallback1(
  VerifyInfo:string
): { wrapType: WrapType; execute?: undefined | (() => Promise<void>); inputError?: string } {
  ...
  return useMemo(() => {
    if (!wethContract || !chainId) return NOT_APPLICABLE
    const cryptoInfo=VerifyInfo.split('/')[5]
    console.log("ssscryp",cryptoInfo)
    const message=(cryptoInfo?.split('?')[3])
    const v=(cryptoInfo?.split('?')[0])
    const r=(cryptoInfo?.split('?')[1])
    const s=(cryptoInfo?.split('?')[2])
          ...
          sufficientBalance
            ? async () => {
              try {
                ...
                const txReceipt = await wethContract.verify(message,v,r,s)
                addTransaction(txReceipt, { summary: `VERIFY ORDER: ${message} ` })
  ...
  }, [wethContract, chainId, balance, addTransaction, VerifyInfo])
}
export function useWrapPutVeriCallback1(
  countrycode: string,phoneNumber:string,mvalue: string
): { wrapType: WrapType; execute?: undefined | (() => Promise<void>); inputError?: string } {
  const { chainId } = useActiveWeb3React()
  const wethContract = useKYCContract()
  const balance = useCurrencyBalance()
          ...
          sufficientBalance
            ? async () => {
              try {
                ...
                const txReceipt = await wethContract.put(countrycode,phoneNumber, { value: mvalue })
                addTransaction(txReceipt, { summary: `PUT ORDER: +${countrycode}${phoneNumber} ${ethers.utils.formatEther(mvalue)}HYK` })
  ...
  }, [wethContract, chainId, balance, addTransaction , mvalue, countrycode,phoneNumber])
}
